...
def I(angle, offset, I_0, w, q):
	theta = angle-offset
	return I_0*(1 + np.exp(-2*theta*theta/w)*np.cos(2*q*theta + np.pi))/2

data = np.genfromtxt("interferencia.txt", usecols = (0, 1), encoding = "latin-1")

angles = np.linspace(np.min(data[:,0]),np.max(data[:,0]),1000)
popt, pcov = curve_fit(I, data[:,0], data[:,1], p0 = [0.53, np.max(data[:,1]), 8, 8.7])
